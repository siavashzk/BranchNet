...
#ifndef PREDICTOR_SIZE
#define PREDICTOR_SIZE 64
#endif

#if PREDICTOR_SIZE == 64
...
public:
  int THRES;
    ...
    PREDICTOR (const char* hard_br_file_path)
  {
    if (hard_br_file_path) {
      std::cout << "Hard br file path: " << std::string(hard_br_file_path) << "\n";
      std::ifstream ifs(hard_br_file_path, std::ifstream::in);
      while (true) {
        uint64_t br_pc;
        ifs >> std::hex >> br_pc;
        if (ifs.eof()) break;
        hard_brs.push_back(br_pc);
      }
      for (auto pc : hard_brs) {
        printf("skipping 0x%lx\n", pc);
      }
    }
...
// computes the TAGE table addresses and the partial tags
    ...
    Tagepred (PC);
#ifdef DEBUG_PRINTS
    std::cout << "Tage pred: " << (int)tage_pred << std::endl; //SIA
#endif
...
#ifdef DEBUG_PRINTS
    std::cout << "Tage or loop pred: " << (int)pred_inter << std::endl;//SIA
    std::cout << "High confidence: " << (int)HighConf  << std::endl;//SIA
    std::cout << "Medium confidence: " << (int)MedConf << std::endl;//SIA
    std::cout << "Low confidence: " << (int)LowConf  << std::endl;//SIA
    std::cout << "longest match prediction: " << (int)LongestMatchPred << std::endl;
    std::cout << "alt prediction: " << (int)alttaken << std::endl;
    std::cout << "alt confidence: " << (int)AltConf  << std::endl;
    std::cout << "hit bank: " << HitBank << std::endl;
    std::cout << "alt bank: " << AltBank << std::endl;
#endif
...
#ifdef DEBUG_PRINTS
    std::cout << "gehl sum: " << LSUM << std::endl;
    std::cout << "thrs sum: " << THRES << std::endl;
#endif
...
//Choser uses TAGE confidence and |LSUM|
		      ...
		      folded_history * J)
  {
	...
	break;
      ...
      default : //spruett: do nothing.. added so code will compile
        break;
...
// PREDICTOR UPDATE
			...
			bool predDir, UINT64 branchTarget)
  {
    ...
    bool skip_hard_br = false;
    for (size_t i = 0; i < hard_brs.size(); ++i) {
      if (PC == hard_brs[i]) {
        skip_hard_br = true;
        break;
      }
    ...
    bool SCPRED = (LSUM >= 0);
    if (pred_inter != SCPRED && !skip_hard_br)
      {
	if ((abs (LSUM) < THRES))
	  if ((HighConf))
	    {
    ...
    if (((SCPRED != resolveDir) || ((abs (LSUM) < THRES))) && !skip_hard_br)
      {
	{
