...
#ifdef ONLY_GTAGE
bool DISABLE_TAGE[6] = {false, true, true, true, true, true};
#else
bool DISABLE_TAGE[6] = {false, false, false, false, false, false};
#endif
...
#define ASSERT_TAGE(cond) if (!(cond)) {fprintf(stderr,"file %s assert line %d\n",__FILE__,__LINE__); abort();}
			  ...
			  int injected_bits)
{
  ...
  ASSERT_TAGE (clength < 32);
  ASSERT_TAGE (nbits <= clength);
  mask1 = (1 << clength) - 1;
  mask2 = (1 << nbits) - 1;
  reset ();
}
...
{
  ASSERT_TAGE (m < clength);
  ASSERT_TAGE ((x >> clength) == 0);
...
PREDICTOR::PREDICTOR (char* unused)
{
...
// global branch history
  ...
  //is it really useful ?
  if (( (long long)(PC + 16) < lastaddr) || ( (long long)(PC) > (lastaddr + 128) ))
    {
      BHIST = (BHIST << 1) ^ (PC & 15);
    }
  lastaddr = PC;
